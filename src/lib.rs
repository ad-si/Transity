...
pub fn parse_ledger_str(yaml: &str) -> Result<Ledger> {
    let raw: LedgerRaw =
        serde_yaml::from_str(yaml).with_context(|| "Cannot parse YAML".to_string())?;
    Ledger::from_raw(raw)
}
...
pub fn align_number(color: bool, int_width: usize, frac_width: usize, f: f64) -> String {
    let s = format!("{}", f);
    let parts: Vec<&str> = s.splitn(2, '.').collect();
...
pub fn account_to_width_record(account_id: &str, map: &CommodityMap) -> WidthRecord {
